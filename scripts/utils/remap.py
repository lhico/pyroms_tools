...
def remap(src_file, src_grd, dst_grd, src_varname, starttime, scalar_dict, 
          dst_file='', strtime='%Y-%m-%dT%H:%M:%S', dmax=0, cdepth=0, kk=0, weight_file=''):
    ...
    if ndim == 3:
        src_var = src_var[::-1, yrange[0]:yrange[1]+1, xrange[0]:xrange[1]+1]
    ...
    if ndim == 3:
        ...
        dst_var = pyroms.remapping.z2roms(dst_varz[:,:,:], dst_grdz, \
                          dst_grd, spval=spval, flood=True,
                          Cpos=Cpos)
    else:
        dst_var = dst_varz
...
def remap_uv(src_file, src_grd, dst_grd, starttime,
    ...
    src_vz = pyroms_toolbox.CGrid_GLORYS.flood(src_varv, src_grd, Cpos='v',
                spval=spval, dmax=dmax, cdepth=cdepth, kk=kk)
    globals()['src_uz'] = src_uz
    globals()['src_vz'] = src_vz
    # horizontal interpolation using scrip weights
    print('horizontal interpolation using scrip weights')
    ...
    dst_vz = pyroms.remapping.remap(src_vz, wts_file_v,
                                      spval=spval)
    globals()['dst_uz'] = dst_uz
    globals()['dst_vz'] = dst_vz
    ...
    # dst_ubar[dst_ubar!=1e+37]=0
    # dst_vbar[dst_vbar!=1e+37]=0
...
def create_scalar_fields_3D(dst_grd, src_varname, starttime, scalar_dict, 
